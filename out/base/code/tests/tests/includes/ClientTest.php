...
\HH\invariant(!is_null($get), '%s', new Error(''));
...
\HH\invariant(      $user instanceof \WP_User,
  ...
  public function testSave2() {
    ...
    $this->assertSame(is_null($user) ? null : $user->get("display_name"), 'HANS WURST');
    $this->assertSame(is_null($user) ? null : $user->get("user_login"), 'billgates'); //stays the old one
  ...
  public function testSave21() {
    ...
    $this->assertTrue(
      wp_check_password('alex', is_null($user) ? null : $user->get('user_pass'), is_null($user) ? null : $user->ID)    );
...
\HH\invariant(!is_null($client), '%s', new Error(''));
