...
$query_args = array()  ){
    ...
    $meta = array("caption" => \is_null($image) ? null : $image->post_excerpt);
  ...
  public static function get_minithumb($id){
    ...
    $medium_exists = \codeneric\phmm\Utils::array_nested_key_exist(
      'sizes.medium.file',
      /*HH_IGNORE_ERROR[4110]*/$imagedata    );
    ...
    if ($medium_exists || $thumb_exists) {
      $size_name =
        $medium_exists ? 'medium' : 'thumbnail'; //prefer medium over thumbnail
...
$query_args = []  ){
    ...
    if ($img_arr === false)
      return null;
\HH\invariant(      \is_array($img_arr) && \count($img_arr) >= 4 && \is_string($img_arr[0]),
      '%s',
