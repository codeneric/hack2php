...
$semaphore_state  ){
    ...
    $sum_done = (\count($semaphore_state['finished']) +
                 \count($semaphore_state['failed']));
    $sum_outstanding = \count($semaphore_state['outstanding']);
...
\HH\invariant(      is_array($functions),
    ...
    \natsort($update_funcs); /*  //sort the functions s.t. they are executed in the correct order! */
...
$funcName  ){
    ...
    $res = null;
    switch ($funcName) {
      case 'update_to_4_2_0':
        $funcContainer->update_to_4_2_0();
