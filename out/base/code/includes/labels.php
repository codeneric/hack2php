...
\HH\invariant(is_array($labels),'%s', new Error('Expected labels to be of type array.'));
    return $labels;
  }
  /**
...
\HH\invariant(is_array($label),'%s', new Error('label expected to be of type array'));
...
\HH\invariant($name !== "",'%s', new Error('Label name cannot be empty string'));
...
$labelID,
$imageIDs  ){
\HH\invariant(is_array($imageIDs),'%s', new Error('Expected labels to be of type array.'));
...
$labelID  ){
  ...
  public static function delete_label($id){
    // get all labels
    $labels = self::get_all_labels();
\HH\invariant(is_array($labels),'%s', new Error('get all labels does not return array!'));
\HH\invariant((count($labels) !== 0),'%s', new Error('_labels is an empty array'));
...
\HH\invariant(is_array($client_ids),'%s', new Error('client_ids is not an array'));
        $project_ids = Project::get_all_ids();
\HH\invariant(is_array($project_ids),'%s', new Error('project_ids is not an array'));
...
\HH\invariant(is_array($res),'%s', new Error('result of get all labels is not an array!'));
    if (count($res) != 0) {
...
\HH\invariant(is_string($label['id']),'%s', new Error('_label id is not a string'));
\HH\invariant(is_string($label['name']),'%s', new Error('_label name is not a string'));
          // add label here
...
\HH\invariant(is_string($val),'%s', new Error('_val name is not a string'));
