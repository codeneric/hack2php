...
class FunctionContainer {
  ...
  public function update_to_1_1_0(){ //update from 1.0.1 to 1.1.0
...
\HH\invariant(is_array($options), '%s', new Error("Expected array."));
  ...
  public function update_to_2_7_0(){
    ...
    if (\is_link("$upload_dir/protect.php"))
      \unlink(
        "$upload_dir/protect.php"      ); //the htaccess redirects to the actual php-file
...
\HH\invariant(      $wpdb instanceof \wpdb,
    ...
    try {
      ...
      $plugin_settings_4_0_0 =
        \codeneric\phmm\legacy\map_plugin_settings_from_3_6_5($plugin_settings);
    ...
    $caller = function($client_id) {
      ...
      try {
        ...
        if (!\is_null($client['wp_user_id'])) {
          ...
          $client_4_0_0 =
            \codeneric\phmm\legacy\map_client_from_3_6_5($client, $project_ids);
          Logger::info(
            "Migrated client to v4.0.0. for ID ".$client_id,
            $client          );
          \codeneric\phmm\legacy\v4_0_0\save_client($client_id, $client_4_0_0);
...
\HH\invariant(      $wpdb instanceof \wpdb,
  ...
  public function update_to_4_2_0(){
...
$cc_phmm_config  ){
    ...
    $p = (bool)\get_option('cc_prem');

    if ($p && !$cc_phmm_config['has_premium_ext']) {
      // FrontendHandler::
      $cc_phmm_base_admin_notice_update_to_premium = function() {
      ...
      $cc_phmm_config['has_premium_ext'] &&
      ...
      $cc_phmm_base_admin_notice_update_to_premium = function() {
        ...
        $script =
          ""; //"<script> function cc_phmm_handle_download_click(e){e.preventDefault();alert('alles ok');} jQuery('#cc_phmm_download_notice').on('click',cc_phmm_handle_download_click); </script>";
        echo
