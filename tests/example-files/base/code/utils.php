...
class Utils {
  ...
  static function close_and_delete_file(resource $handle, string $name): bool {
  ...
  static function get_intermediate_image_sizes(): array<string> {
  ...
  > {
    ...
    foreach (self::get_intermediate_image_sizes() as $_size) {
      if (
        ...
        $sizes[$_size] = shape('width' => 0, 'height' => 0, 'crop' => true);
  ...
  static function add_admin_notice(
    ...
    $f = function() use ($html, $dismissable, $type) {
      $dis = $dismissable ? 'is-dismissible' : '';
      echo "<div class=\"notice notice-$type $dis \">$html</div>";

    };
    \add_action('admin_notices', $f);
  }

  static function get_protocol_relative_url(string $url): string {
