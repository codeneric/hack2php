...
class FrontendHandler {
  ...
  private static function is_client_single_page(){
    return self::is_custom_post_edit_or_new() &&
  ...
  private static function is_our_business(){
    ...
    $config = Configuration::get();
    return $current_post_type === $config['client_post_type'] ||
  ...
  public static function enqueue_scripts($hook){
    ...
    // product tour
    if ("%%DEMO_BUILD%%" === "true") {
      if (self::should_start_product_tour()) {
        $scriptsrc = $configuration['assets']['js']['admin']['product_tour'];
        $scripthandle = $configuration['plugin_name']."-admin-producttour";
        ...
        \wp_localize_script(
          $scripthandle,
          'codeneric_phmm_product_tour_should_start',
          \json_encode(self::should_start_product_tour())        );
    ...
    if (self::is_client_single_page()) {
      ...
      \wp_localize_script(
        ...
        \json_encode(self::get_admin_client_project_access_frontend_globals())      );
  ...
  public static function handle_analytics_enqueue(){
    ...
    $trackingID = Configuration::get()['ga_tracking_id'];

    $gTagSnippet = $settings['analytics_opt_in']
      ? "<script async src='https://www.googletagmanager.com/gtag/js?id=$trackingID'></script>"
      : "";
    $optOut = $settings['analytics_opt_in'] === true
  ...
  public static function handle_placeholder_image_upload(){
    ...
    $attachment = array(
      ...
      'post_title' =>
        \preg_replace('/\.[^.]+$/', '', \basename($upload['file'])),
      'post_content' => '',
      'post_status' => 'inherit',
    );
  ...
  public static function get_general_admin_frontend_globals(
    ...
    $res = array(
      ...
      "canned_emails_supported_placeholders" =>
        \array_values(CannedEmailPlaceholders::getValues()),
...
\HH\invariant(      !\is_null($client),
  ...
  public static function get_admin_client_project_access_frontend_globals(
    ...
    foreach ($posts as $key => $project) {
      $configurations[$project->ID] = Project::get_configuration($project->ID);
    }
...
$posts  ){
  ...
  public static function hide_admin_bar(){
    ...
    if (
      $settings['hide_admin_bar'] === true && !\current_user_can('edit_posts')
    )
...
\HH\invariant(              is_string($editLink),
              '%s',
              new Error("Could not get project post edit link from post_id"));

            echo '<strong><a class="row-title post-edit-link" href="'.
              $editLink.
              '">'.
              self::string_or_else(
                $project->post_title,
...
$html  ){
    ...
    if (
      $postType !== $config['client_post_type'] &&
  ...
  public static function should_start_product_tour(){
    $config = Configuration::get();
    $option_product_tour_started = $config['option_product_tour_started'];
