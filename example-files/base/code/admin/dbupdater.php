...
class DBUpdater {
  ...
  static string $mutex_name = "codeneric_phmm_migration";
  static string $highlevel_mutex_name = "codeneric_phmm_migration_highlevel";
  ...
  private static function get_outstanding_functions(
    ...
    $filterFunctions = function(string $v): bool {
  ...
  static function version_to_func_name(string $newVersion): string {
  ...
  static function update(\codeneric\phmm\type\configuration\I $config): num {
    ...
    if ($mutex->acquireLock(0)) {
      ...
      invariant(
        ...
        '%s',
        new Error('String expected', [tuple('oldVersion', $oldVersion)]),
      );
      ...
      $caller = function(string $funcName) use ($config) {
  ...
  private static function execute_update_function<T>(
    string $funcName,
  ): SemaphoreExecutorReturn {
    $funcContainer = new FunctionContainer();
    $res = null;
    switch ($funcName) {
