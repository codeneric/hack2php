...
class Main {
  ...
  private static ?array<string, string>
    $CACHE_resume_photo_upload_dir = null;
  public static function update_database(): void {
    $config = Configuration::get();
    \codeneric\phmm\DBUpdater::update($config);
  }
  public static function register_client_post_type(): void {
    $config = Configuration::get();
  ...
  public static function register_project_post_type(): void {
    $config = Configuration::get();
  ...
  public static function add_client_meta_box(): void {
    $config = Configuration::get();
    $pluginName = $config['plugin_name'];
    add_meta_box(
  ...
  public static function save_meta_box_data(
    ...
    invariant(
      is_string($post_type),
      '%s',
      new Error('Post type not string', [tuple('post_type', $post_type)]),
    );
  ...
  public static function cleanup_before_deletion(int $projectID): void {
  ...
  private static function parse_query(): array<string, string> {
    $query_params = array();
  ...
  private static function request_comes_from_project_edit_page(): bool {
  ...
  private static function download_proofing_csv(
    \codeneric\phmm\type\get_proofing_csv $request,
  ): void {
  ...
  public static function provide_csv(): void {
